---
title: "AutoPref: Automatic Discovery of Task-Specific Preference Objectives for Neural Combinatorial Optimization"
title_zh: AutoPref：神经组合优化中任务特定偏好目标的自动发现
authors: "Shengda Gu, Kai Li, Xinyi Ke, Haobo Fu, Yifan Zhang, Jian Cheng"
date: 2026-07-30
pdf: "https://arxiv.org/pdf/2607.27953v1"
tags: ["query:llm-opt-code"]
score: 7.0
evidence: LLM引导的神经组合优化偏好目标自动发现框架
tldr: 组合优化问题解空间巨大，神经组合优化（NCO）结合偏好学习可提升样本效率，但现有偏好目标依赖手工设计，无法适应任务差异。本文提出AutoPref，首个基于大语言模型（LLM）引导的自动化偏好目标发现框架，将目标分解为成对损失与集合感知加权两个程序组件，并组合成统一目标空间。通过带行为门控的阶段化条件搜索，在训练前过滤低效程序，显著降低搜索成本。在TSP、CVRP、FFSP和JSSP等多个问题及不同规模上，AutoPref一致超越手工设计的强基线，展示了自动发现任务特定目标的优越性和可扩展性。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2607.27953v1/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1611, \"height\": 704, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2607.27953v1/fig-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 855, \"height\": 568, \"label\": \"Figure\"}]"
tables_json: "[{\"url\": \"assets/tables/arxiv/2607.27953v1/table-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 619, \"height\": 246, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.27953v1/table-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1841, \"height\": 419, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.27953v1/table-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 1840, \"height\": 356, \"label\": \"Table\"}]"
motivation: 现有人工设计的偏好目标在NCO中缺乏任务自适应性，无法高效利用相对解质量信息，需要自动化目标发现。
method: AutoPref将偏好目标分解为成对损失与集合感知加权程序，利用LLM引导生成，并用行为门控阶段化搜索筛选。
result: 在TSP、CVRP、FFSP和JSSP上，AutoPref跨问题规模一致优于手工设计基线。
conclusion: 首次实现NCO偏好目标自动化发现，验证LLM驱动目标搜索的有效性与可扩展性。
---

## 摘要
组合优化问题（COPs）支撑着许多现实世界的决策，但其指数级的搜索空间使得获取高质量解决方案的成本高昂。神经组合优化（NCO）通常通过强化学习（RL）学习快速构建策略，而基于偏好的NCO通过从相对解决方案质量中学习来提高样本效率。然而，现有的偏好目标在手动指定的通用公式中结合了两个不同的设计选择：从每个解决方案对中提取什么学习信号，以及如何相对于采样集合对每对进行加权。我们提出了AutoPref，这是第一个在NCO中用于自动化偏好目标发现的LLM引导框架。AutoPref将目标分解为一个成对损失程序（定义学习信号）和一个集合感知加权程序（确定每对的相对贡献）。它们的组合形成了一个统一的程序化目标空间，其中现有的偏好目标是特殊情况。为了使搜索易于处理，我们引入了一种分阶段的条件搜索策略，带有行为门控，在短时训练和评估之前过滤不可接受的程序。在TSP、CVRP、FFSP和JSSP上，AutoPref在不同问题规模上始终优于强大的手工设计基线，证明了自动化目标发现对NCO的益处和可扩展性。

## Abstract
Combinatorial optimization problems (COPs) underpin many real-world decisions, but their exponentially large search spaces make high-quality solutions costly to obtain. Neural combinatorial optimization (NCO) learns fast construction policies, typically with reinforcement learning (RL), while preference-based NCO improves sample efficiency by learning from relative solution quality. However, existing preference objectives combine two distinct design choices in manually specified, one-size-fits-all formulations: what learning signal to extract from each solution pair and how to weight each pair relative to the sampled set. We present AutoPref, the first LLM-guided framework for automated preference-objective discovery in NCO. AutoPref factorizes the objective into a pairwise loss program, which defines the learning signal, and a set-aware weighting program, which determines each pair's relative contribution. Their composition forms a unified programmatic objective space containing existing preference objectives as special cases. To make its search tractable, we introduce a staged conditional search strategy with behavioral gates that filter inadmissible programs before short-horizon training and evaluation. Across TSP, CVRP, FFSP, and JSSP, AutoPref consistently outperforms strong hand-designed baselines across problem scales, demonstrating the benefits and scalability of automated objective discovery for NCO.